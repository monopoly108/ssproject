...
class OrderSessionEventHandler(object):
    """
    Class for trade sessioon verification
    """
    query_state = 0
...
def bid_order(login_gb="", account_nm="", order_pw="", gicode="",
              order_qty=int(), order_prc=int(), bid_gb='2', order_type="03",
              credit_gb="000", order_con='0', res_id="CSPAT00600"):
    """
    Buy(bid) Order
    ...
    """
    order = win32com.client.DispatchWithEvents(_QUERY_NM,
                                               OrderSessionEventHandler)
    order.ResFileName = _RES_FILE_NM.replace("tmpnm", res_id)

    order.SetFieldData(_IN_BLOCK1.replace("tmpnm", res_id),
                       'AcntNo', 0, account_nm)
    order.SetFieldData(_IN_BLOCK1.replace("tmpnm", res_id),
                       'InptPwd', 0, order_pw)
    if login_gb == '0':
        ...
        raise ValueError
    order.SetFieldData(_IN_BLOCK1.replace("tmpnm", res_id), 'OrdQty', 0,
                       int(order_qty))
    if order_type == "03":
    ...
    order.SetFieldData(_IN_BLOCK1.replace("tmpnm", res_id), 'BnsTpCode',
                       0, bid_gb)
    order.SetFieldData(_IN_BLOCK1.replace("tmpnm", res_id),
    ...
    OrdNo = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                               'OrdNo', 0)  # 주문번호
    IsuNo = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                               'IsuNo', 0)  # 종목코드
    OrdQty = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                'OrdQty', 0)  # 종목수량
    OrdPrc = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                'OrdPrc', 0)  # 종목수량
    OrdAmt = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                'OrdAmt', 0)  # 주문금액
    BnsTpCode = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                   'BnsTpCode', 0)  # 종목수량
    OrdTime = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                 'OrdTime', 0)  # 주문시각
...
def ask_order(login_gb="", account_nm="", order_pw="", gicode="",
              order_qty=int(), order_prc=int(), bid_gb='1', order_type="03",
              credit_gb="000", order_con='0', res_id="CSPAT00600"):
    """
    Ask(sell) Order
...
def cancel_order(login_gb="", account_nm="", order_pw="",
                 order_num="", gicode="", order_qty=int,
                 res_id="CSPAT00800"):
    """
    Cancel order
    ...
    OrdNo = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                               'OrdNo', 0)  # 주문번호
    PrntOrdNo = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                   'PrntOrdNo', 0)  # 모주문번호
    OrdTime = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                 'OrdTime', 0)  # 주문시각
    OrdPtnCode = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                    'OrdPtnCode', 0)  # 주문유형코드
    BnsTpCode = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                                   'BnsTpCode', 0)  # 매매구분
    IsuNo = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                               'IsuNo', 0)  # 종목코드
    IsuNm = order.GetFieldData(_OUT_BLOCK2.replace("tmpnm", res_id),
                               'IsuNm', 0)  # 종목명
    OrdQty = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                'OrdQty', 0)  # 주문수량
...
def revise_order(login_gb="", account_nm="", order_pw="",
    ...
    OrdPrc = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                'OrdPrc', 0)  # 주문가격
    OrdprcPtnCode = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
                                       'OrdprcPtnCode', 0)  # 호가유형구분
    OrdCndiTpCode = order.GetFieldData(_OUT_BLOCK1.replace("tmpnm", res_id),
